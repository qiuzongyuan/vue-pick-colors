...
## 设置格式选项
`format-options` 设置为 `false` <pick-colors v-model:value="formatValue" :format="format" :format-options="false" show-alpha/>
...
`format-options` 设置为 `['rgb', 'hex']` <pick-colors v-model:value="formatValue" :format="format" :format-options="['rgb', 'hex']" show-alpha/>
